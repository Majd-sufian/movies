...
window.onload = function() {
  document.getElementById("movies-submit").addEventListener("click", all);
}

function all(){
  var sort = document.getElementById('sort-type').value;
  let sortMovies = sortMoviesByAttr(movies, sort)
...
function sortMoviesByRank(movies){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
    ...
    //   index
    let max_location = j;
    // the num from 0 to movies.length

    for (let i = j; i < movies.length; i++) {
        // we should access the variable with [] and not .
        // if you want to get elements form smaller to bigger just write (< instead >)
        if (movies[i].rank > max_obj.ra) {
 ...
 * @param sortAttr pass in 'id', 'txitle', or 'rank' to sort by
 */
